...
func NewHandler(k keeper.Keeper) sdk.Handler {
	...
	return func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		...
		switch msg := msg.(type) {
		case *types.MsgUploadReport:
			res, err := msgServer.UploadReport(sdk.WrapSDKContext(ctx), msg)
		...
		case *types.MsgSetProfile:
			...
			return sdk.WrapServiceResult(ctx, res, err)
			case *types.MsgSendRecommend:
					res, err := msgServer.SendRecommend(sdk.WrapSDKContext(ctx), msg)
					return sdk.WrapServiceResult(ctx, res, err)
